...
const saveLoad = {
  ...
  saveTheWorld:function(comment) {
    const base = saveLoad.getSaveHeader(comment) + saveLoad.getSaveBody();
    if (typeof settings.save === 'function') {
      const payload = settings.save();
      if (payload && typeof payload === 'object') {
        return base + '!CustomState=' + saveLoad.encodeString(JSON.stringify(payload));
      }
    }
    return base;
  },
  ...
  loadTheWorld:function(s, removeHeader) {
    ...
    const customState = arr.find(el => el.startsWith('CustomState='));
    if (customState) {
      const text = customState.substring('CustomState='.length)
      const state = JSON.parse(saveLoad.decodeString(text))
    ...
    for (let el of arr) {
      if (el.startsWith('CustomState=')) continue;
